...
module.exports = class $fireflyio
{
    ...
    this.io.on('connection', client =>
    {
      ...
      ['GET', 'DELETE', 'POST', 'PUT'].map(method =>
        client.on(method, async data => {
          this.debug('[DEBUG]', 'FIREFLYIO: new request', method, data.location, data.uuid);
          const toEmit = await this._processEvent(method, data)
  ...
  async _processEvent(method, data) {
    const route = this._retrieveRouteMatch(method, data.location);
    if (!route) {
      return this._setResponseError(data.uuid, '404');
    }
    const ctx = {
    ...
    return this._setResponse(data.uuid, ctx);
  }
